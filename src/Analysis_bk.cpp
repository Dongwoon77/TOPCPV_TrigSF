...
void Analysis::Loop() {
	...
	current_entry_ = ientry;
        ...
        //FillHisto( h_DiLepMass[1], ( (Lep1)+(Lep2) ).M());
        FillHisto( h_Num_PV[1],     num_pv);
        //FillHisto( h_Lep1pt[1] ,    (Lep1).Pt());
        //FillHisto( h_Lep1eta[1],    (Lep1).Eta() );
        //FillHisto( h_Lep1phi[1],    (Lep1).Phi() );
        //FillHisto( h_Lep2pt[1] ,    (Lep2).Pt() );
        //FillHisto( h_Lep2eta[1],    (Lep2).Eta() );
        //FillHisto( h_Lep2phi[1],    (Lep2).Phi());
        //FillHisto( h_METpt[1]   ,   Met.Pt()  );
        //FillHisto( h_METphi[1]  ,   Met.Phi()  );
        //FillHisto( h_Num_Jets[1]  , v_jet_idx.size());
        //FillHisto( h_Num_bJets[1], nbtagged);
    ...
    // Dielectron channel
        ...
        for (int i = 0; i < nel; ++i) {
            ...
            if (!passKinematicCuts(elecs_pt->At(i), elecs_eta->At(i), elec_pt, elec_eta) ||
                ...
                !(elecCharge((*ucharVectors["Electron_tightCharge"])[i]))) {
                continue;
            }
...
void Analysis::MakeJetCollection() {
    ...
    JetCorrectionOutput corr_output = SSBCorr->ApplyJetCorrectionsWithMET(
        ...
        // When corrections are disabled, use raw values
        corr_output.corrected_jets = rawJets;
        corr_output.corrected_met = TLorentzVector();
        corr_output.corrected_met.SetPtEtaPhiM(raw_met_pt, 0, raw_met_phi, 0);
    }
    ...
    if (applyMETXY == "True" && SSBCorr) {
        Met = SSBCorr->METXYCorrection(Met, **uintSingles["run"], yearForm, isMC, **intSingles["PV_npvsGood"], isUL, isPuppi);
    }
...
void Analysis::JetSelector() {
	...
	// Initialize jet veto event flag
        ...
        if (should_apply_hem && SSBCorr && SSBCorr->ShouldVetoJet(jetVec)) {
    ...
    if (TString(Decaymode).Contains("dimuon")) {
        ...
        for (int iel = 0; iel < nel; ++iel) {
            ...
            if (!passKinematicCuts(elecs_pt->At(iel), elecs_eta->At(iel), elec_pt, elec_eta) ||
                ...
                !elecCharge((*ucharVectors["Electron_tightCharge"])[iel]) ||
    ...
    if (isData){
        ...
        if (TString(Decaymode).Contains("dimuon")) {
            ...
            if (SSBCorr) {
                ...
                lep_sf = 1.0; // Default value when corrections are disabled
            }
...
void Analysis::PUWeightApply()
{
    ...
    {  
       if (SSBCorr) {
           ...
           puweight_ = 1.0; // Default value when corrections are disabled
       }
...
{        
    ...
    if (!TString(FileName_).Contains("Data")) {
        ...
        if (SSBCorr) {
            ...
            triggersf_ = 1.0; // Default value when corrections are disabled
        }
        else {
...
void Analysis::BTaggingSFApply() {
    ...
    try {
        if (SSBCorr) {
            ...
            btag_sf_weight_ = 1.0; // Default value when corrections are disabled
        }
...
void Analysis::PUIDSFApply() {
    ...
    for (const auto& jet_info : puid_hardscatter_jets_) {
        try {
            ...
            float eff = 1.0;
            if (SSBCorr) {
                eff = SSBCorr->GetPUJetIDSFAndEff(
