...
//    std::string jsonDir = std::filesystem::current_path().string() + "/jsonpog-integration/POG/";
    ...
    // PU weights are optional for some Run3 eras; load only if available and key exists
    pu_weight_ = nullptr;
    if (!puw_path.empty()) {
    ...
    // JMAR (PUJetID) SFs are optional (e.g., not provided for some Run3 eras). Load only if available.
    pujetid_sf_ = nullptr;
    if (!jmar_path.empty()) {
        std::string jmar_full = jsonDir + jmar_path;
        if (std::filesystem::exists(jmar_full)) {
            try {
                auto jmar_sf_set = correction::CorrectionSet::from_file(jmar_full);
                pujetid_sf_ = jmar_sf_set->at("PUJetID_eff");
            } catch (const std::exception& e) {
                std::cerr << "[WARNING] Failed to load JMAR (PUJetID) corrections from " << jmar_full
                          << ": " << e.what() << std::endl;
                pujetid_sf_ = nullptr;
            }
        } else {
            std::cout << "[INFO] JMAR file not found at " << jmar_full << " — skipping PUJetID SFs." << std::endl;
        }
    } else {
        std::cout << "[INFO] JMARPath empty — skipping PUJetID SFs." << std::endl;
    }
    ...
    std::string btag_sf_json = "BTV/" + year_ + "_Summer24/btagging.json.gz";
    std::string btag_tagger = "UParTAK4";
    ...
    // For UParTAK4 in Run3, SFs are not provided; skip SF/eff initialization
    if (btag_algo == "UParTAK4") {
        static bool btag_warned_once = false;
        if (!btag_warned_once) {
            std::cout << "[INFO] UParTAK4 selected — skipping b-tag SF/eff initialization (Run3 provides only WP values)." << std::endl;
            btag_warned_once = true;
        }
    } else {
...
float SSBCorrections::GetPUJetIDSFAndEff(float pt, float eta, bool passPU, bool genMatched, const std::string& wp, const std::string& syst, bool getEff) const {
    if (!pujetid_sf_) {
        ...
        if (!warned_once) {
            std::cerr << "[INFO] PUJetID correction (JMAR) not loaded — returning 1.0. Further messages suppressed." << std::endl;
            warned_once = true;
        }
        return 1.0;
    }
...
float SSBCorrections::GetPUWeight(float nTrueInt, const std::string& systTag) const {
    ...
    if (!pu_weight_) {
        static bool warned_once = false;
        if (!warned_once) {
            std::cerr << "[INFO] PU weights not loaded — returning 1.0. Further messages suppressed." << std::endl;
            warned_once = true;
        }
        return 1.0;
    }
...
void SSBCorrections::InitBtagSFCorrection(const std::string& json_path, 
    ...
    if (pos != std::string::npos) {
        ...
        // For Run3 UParTAK4 (no SF nodes), skip quietly
        std::cout << "[INFO] Tagger name has no 'comb' (" << tagger_name
                  << ") — skipping b-tag SF initialization for this tagger." << std::endl;
        return;
    }
...
void SSBCorrections::LoadMCBtagEfficiencies(const std::string& filepath, const std::string& algo) {
    static bool warned_once_missing = false;
    if (!std::filesystem::exists(filepath)) {
        if (!warned_once_missing) {
            std::cout << "[INFO] BTag efficiency file not found at " << filepath
                      << " — skipping. Further messages suppressed." << std::endl;
            warned_once_missing = true;
        }
        return;
    }
    ...
    if (!f || f->IsZombie()) {
        if (!warned_once_missing) {
            std::cerr << "[WARNING] Failed to open efficiency file: " << filepath
...
std::string SSBCorrections::ExpandJECName(const std::string& base_jec_name, const std::string runPeriod, const std::string& era, bool is_data) {
    // If already fully specified (contains the full L1L2L3Res suffix), return as-is
    if (base_jec_name.find("_L1L2L3Res_") != std::string::npos) {
        return base_jec_name;
    }
